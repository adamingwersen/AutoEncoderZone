...
model_name = 'vgg16_fc1_deeper_dae'
...
""" Extend VGG16 w. Deep Regularized Denoising Autoencoder """
# Model hyperparams
noise_factor = 0.1
regularization = K.regularizers.l1(1e-5)
bottleneck_size = 800
n_epochs = 2000
batch_size = 128
...
encoded = K.layers.Dense(6000, activation = 'relu', activity_regularizer=regularization)(input_layer)
encoded = K.layers.Dense(4000, activation = 'relu')(encoded)
encoded = K.layers.Dense(3000, activation = 'relu')(encoded)
encoded = K.layers.Dense(2000, activation = 'relu')(encoded)
encoded = K.layers.Dense(bottleneck_size, activation = 'elu')(encoded)

decoded = K.layers.Dense(2000, activation = 'relu')(encoded)
decoded = K.layers.Dense(3000, activation = 'relu')(decoded)
decoded = K.layers.Dense(4000, activation = 'relu')(decoded)
decoded = K.layers.Dense(6000, activation = 'relu')(decoded)
decoded = K.layers.Dense(autoencoder_input, activation = 'softmax')(decoded)

vgg16_fc1_deeper_dae = K.models.Model(input_layer, decoded)
encoder = K.models.Model(input_layer, encoded)

vgg16_fc1_deeper_dae.compile(optimizer = 'adadelta', loss = 'mse')

callbacks = utils.get_callbacks(model_name)

vgg16_fc1_deeper_dae.fit(train_feature_vecs, train_feature_vecs,
...
""" Save the Model """
vgg16_fc1_deeper_dae.save(weights_path + model_name + '.hdf5')
